export namespace IndexedDbUtils {
  export function promisifyRequest<T = unknown>(request: IDBRequest<T> | IDBTransaction): Promise<T> {
    return new Promise<T>((resolve, reject) => {

      (request as any).oncomplete = (request as any).onsuccess = () => resolve((request as any).result);

      (request as any).onabort = request.onerror = () => reject(request.error);
    });
  }
  ...
  export function createStore(dbName: string, storeName: string): UseStore {
    ...
    const dbp = promisifyRequest<IDBDatabase>(request as any);
  ...
  export function get<T = any>(key: IDBValidKey, customStore = defaultGetStore()): Promise<T | undefined> {
    return customStore('readonly', store => promisifyRequest<T>(store.get(key)));
  }
  ...
  export function entries<KeyType extends IDBValidKey, ValueType = any>(customStore = defaultGetStore()): Promise<[KeyType, ValueType][]> {
    return customStore('readonly', store => {
      ...
      if (store.getAll && store.getAllKeys) {
        return Promise.all([promisifyRequest(store.getAllKeys() as unknown as IDBRequest<KeyType[]>), promisifyRequest(store.getAll() as IDBRequest<ValueType[]>)]).then(
